...
function Board(props) {
  ...
  function digCell(event, x, y) {
    ...
    if (gameStatus === "New game") setGameStatus("Playing...");
  ...
  function getHidden(data) {
    ...
    data.forEach((row) => {
      row.forEach((cell) => {
        if (!cell.isRevealed && !cell.isFlagged && cell.isMine) hiddenMines++;
      });
    });
    return hiddenMines;
  }
  ...
  function RenderInfo() {
    return (
      <div className="board text-center">
        <div>
          mines left: {mineCount}. &nbsp; game status: {gameStatus}.
        </div>
        <div className="btn btn-small btn-primary" onClick={initBoardData}>
          Restart
        </div>
      </div>
    );
  ...
  return (
    <Card>
      <Card.Header>{RenderInfo()}</Card.Header>
      <Card.Body>
...
function Cell(props) {
  ...
  function getCell() {
    ...
    if (cell.isMine) return <FontAwesomeIcon icon="bomb" />;
  ...
  if (cell.isRevealed) {
    ...
    if (cell.isMine) cell.className += " mine";
  }
